            ...
            function downloadGTFS(task, cb) {
                ...
                function importFiles(cb) {
                    ...
                    async.forEachSeries(GTFSFiles, function (GTFSFile, cb) {
                        ...
                        // records counter
                        var cpt_line = 0;
                        // measure callback process duration
                        var mid_time_start = process.hrtime();
                        if (!fs.existsSync(filepath)) {
                            log('Transit network "' + network_key + '" : Importing data - No ' + GTFSFile.fileNameBase + ' file found in ' + filepath);
                            return cb();
                        }

                        log('Transit network "' + network_key + '" : Importing data - ' + GTFSFile.fileNameBase);
                        db.collection(GTFSFile.collection, function (e, collection) {
                            ...
                            parser.on('readable', function () {
                                    while (line = parser.read()) {
                                        ...
                                        //convert fields that should be int depending on current collection
